# Website Journals - Actual Published Data

**Last Updated**: 2026-01-23  
**Source**: Actual website content and published data files  
**Purpose**: Primary source of truth for journal information on newtifi.com

---

## ⚠️ IMPORTANT: Data Source Rules

1. **Use this file as PRIMARY SOURCE** - Do not hallucinate or assume information
2. **Only document what is ACTUALLY published** on the website
3. **Do not add titles/credentials** unless explicitly shown in published content
4. **Verify against actual website** before making claims
5. **Update this file** when new content is published

---

## Active Journal: Investment Management Journal

### Basic Information
- **ID**: `investment-management`
- **Name**: Investment Management Journal
- **Full Name**: NewTIFI Publishing – Investment Management Journal
- **URL**: `/publishing/investment-management`
- **Status**: Active
- **Launch Date**: 2025-06-28
- **Publisher**: New Technologies & Investment Funds Institute
- **Publisher Location**: Luxembourg
- **Frequency**: Quarterly
- **ISSN**: TBD (To be registered with ISSN International Centre)
- **Peer Review**: Double-blind peer review
- **Archiving**: Digital preservation through CLOCKSS and Portico
- **Indexing**: DOAJ, Google Scholar, ResearchGate

### Editorial Board (As Published on Website)
**Source**: `src/pages/publishing/journals/investment-management.tsx` (lines 17-20)

1. **Ezechiel Havrenne**
   - Role: Editor-in-Chief
   - Affiliation: NewTiFi Institute
   - Note: No "Dr." title shown on website

2. **Delphine Filsack**
   - Role: Associate Editor
   - Affiliation: NewTiFi Institute

3. **Karlo Definis**
   - Role: Managing Editor
   - Affiliation: NewTiFi Institute
   - Note: No "Dr." title - user confirmed not a doctor

### Journal Description (As Published)
**Source**: `src/pages/Publishing.tsx` (lines 292-309)

> "The NewTIFI Publishing – Investment Management Journal is a peer-reviewed academic publication, dedicated to advancing rigorous scholarship and high-impact analysis in the field of investment management.
> 
> With a focus on legal, tax, regulatory, and operational dimensions, the journal brings together leading academics, practitioners, and policymakers to explore the evolving landscape of alternative investment funds. It covers topics such as fund structuring, asset management regulation, fiduciary duty, tax policy, compliance and risk management, and financial innovation.
> 
> Our mission is to foster intellectually independent and practically relevant research that informs the decisions of fund managers, investors, depositaries and central administrators, auditors, regulators, and courts worldwide.
> 
> The NewTIFI Publishing – Investment Management Journal welcomes original articles, case studies, and comparative analysis that contribute to a deeper understanding of global investment frameworks and the financial architecture shaping the future of capital markets.
> 
> As part of NewTIFI Publishing, the journal operates under a non-profit model. All profits realised by NewTIFI including through publication efforts are used to fund Doctoral Scholarships."

### Peer Review Process
- **Average Review Time**: 6-8 weeks
- **Acceptance Rate**: 15-20%
- **Review Criteria**:
  1. Originality
  2. Methodology
  3. Contribution to field
  4. Clarity of presentation

---

## Published Articles

### Article 1: IMJ-2025-001
**Source**: `data/articles/investment-management/IMJ-2025-001.json`

- **ID**: IMJ-2025-001
- **Title**: "Closed-Ended Luxembourg ELTIFs Compulsory Redemption Matters and Compartment Termination & Amalgamation Provisions"
- **Author**: Ezechiel Havrenne (corresponding author)
- **Published Date**: 2025-06-28
- **Submitted Date**: 2025-06-28
- **DOI**: 10.1234/newtifi.2025.001
- **Status**: published
- **Version**: v1 (current)
- **Peer Reviewed**: true
- **License**: All Rights Reserved
- **Subject Areas**: Investment Funds, Regulatory Framework
- **Keywords**: ELTIFs, Luxembourg, Compulsory Redemptions, Fund Management
- **Abstract**: "This article examines the legal and regulatory framework governing compulsory redemptions and compartment terminations in Luxembourg closed-ended ELTIFs. Focusing on the interplay between EU law, Luxembourg product regimes, and CSSF practice, it analyses how these mechanisms enhance capital efficiency, support fund liquidity management, and ensure investor protection."
- **File Path**: `/articles/investment-management-journal/2025.06.28_NewTIFI Investment Management Journal - Closed-Ended Luxembourg ELTIFs- Compulsory Redemptions and Compartment Termination & Amalgamation Provisions_Final.pdf`
- **Storage**: `storage/journals/investment-management/articles/IMJ-2025-001/v1/`

### Article 2: IMJ-2025-002
**Source**: `data/articles/investment-management/IMJ-2025-002.json`

- **ID**: IMJ-2025-002
- **Title**: "Investor Oversight or Undue Influence? Reassessing BaFin's Stance on AIFM Portfolio Control"
- **Author**: Ezechiel Havrenne (corresponding author)
- **Published Date**: 2025-06-28
- **Submitted Date**: 2025-06-28
- **DOI**: 10.1234/newtifi.2025.002
- **Status**: published
- **Version**: v1 (current)
- **Peer Reviewed**: true
- **License**: All Rights Reserved
- **Subject Areas**: Fund Management, Regulatory Compliance
- **Keywords**: AIFM, BaFin, Portfolio Control, Investor Oversight
- **Abstract**: "This article critically examines the March 2025 Draft Position Letter issued by BaFin on investor involvement in AIF portfolio decisions. While reaffirming the AIFM's exclusive mandate under the AIFMD, BaFin's strict stance on veto rights, LPAC involvement, and investor oversight diverges from more pragmatic regulatory approaches in other EU jurisdictions."
- **File Path**: `/articles/investment-management-journal/2025.06.28_NewTIFI Investment Management Journal - Investor Oversight or Undue Influence Reassessing BaFin's Stance on AIFM Portfolio Control_Final.pdf`
- **Storage**: `storage/journals/investment-management/articles/IMJ-2025-002/v1/`

### Article 3: IMJ-2025-003
**Source**: `data/articles/investment-management/IMJ-2025-003.json`

- **ID**: IMJ-2025-003
- **Title**: "Luxembourg SICARs, SIFs, and RAIFs: A 20-year Perspective on the Well-Informed Investor Notion"
- **Author**: Ezechiel Havrenne (corresponding author)
- **Published Date**: 2025-06-28
- **Submitted Date**: 2025-06-28
- **DOI**: 10.1234/newtifi.2025.003
- **Status**: published
- **Version**: v1 (current)
- **Peer Reviewed**: true
- **License**: All Rights Reserved
- **Subject Areas**: Investment Vehicles, Investment Funds, Regulatory Framework
- **Keywords**: SICARs, SIFs, RAIFs, Well-Informed Investor, Luxembourg
- **Abstract**: "This article provides a comprehensive analysis of Luxembourg's \"Well-Informed Investor\" regime as applied to SICARs, SIFs, and RAIFs, tracing its legislative and regulatory evolution over the past two decades."
- **File Path**: `/articles/investment-management-journal/2025.06.28_NewTIFI Investment Management Journal - Luxembourg SICARs, SIFs and RAIFs - A 20-year Perspective on the Well-Informed Investor notion_Final.pdf`
- **Storage**: `storage/journals/investment-management/articles/IMJ-2025-003/v1/`

---

## Article Statistics

- **Total Published Articles**: 3
- **All Articles Status**: published
- **All Articles Peer Reviewed**: true
- **All Articles Published Date**: 2025-06-28
- **Unique Authors**: 1 (Ezechiel Havrenne)
- **All Articles Version**: v1 (current)

---

## File Structure

### Storage Structure
```
storage/journals/investment-management/articles/
  ├── IMJ-2025-001/
  │   ├── current/ (symlink to v1)
  │   └── v1/
  │       ├── article.pdf
  │       └── metadata.json
  ├── IMJ-2025-002/
  │   └── (same structure)
  └── IMJ-2025-003/
      └── (same structure)
```

### Data Files
```
data/articles/investment-management/
  ├── IMJ-2025-001.json
  ├── IMJ-2025-002.json
  └── IMJ-2025-003.json
```

### Configuration Files
```
config/journals.json - Journal metadata
```

---

## Routes & URLs

### Website Routes
- **Journal Main Page**: `/publishing/investment-management`
- **Article Pages**: `/publishing/investment-management/article/{slug}`
- **Publishing Hub**: `/publishing` (journals tab)

### Article Slugs (URL-friendly)
- IMJ-2025-001 → `eltifs-compulsory-redemptions`
- IMJ-2025-002 → `bafin-portfolio-control`
- IMJ-2025-003 → `luxembourg-well-informed-investor`

---

## Future Journals

### Planned: FinTech Innovation Journal
**Status**: Planned (not yet active)
**Note**: Do not document details until actually published on website

---

## Important Notes for AI/Developers

1. **Do NOT add titles** (Dr., Prof., etc.) unless explicitly shown in published content
2. **Do NOT assume credentials** - only use what's published
3. **Verify against actual website** before making claims
4. **Update this file** when new articles/journals are published
5. **Use article JSON files** as source of truth for article data
6. **Use page components** as source of truth for displayed information
7. **When in doubt, check the actual website** at newtifi.com

---

## Data Sources Reference

- **Journal Metadata**: `src/pages/publishing/journals/investment-management.tsx` (journalMetadata object)
- **Editorial Board**: `src/pages/publishing/journals/investment-management.tsx` (lines 17-20)
- **Journal Description**: `src/pages/Publishing.tsx` (lines 292-309)
- **Article Data**: `data/articles/investment-management/*.json`
- **Article Storage**: `storage/journals/investment-management/articles/`
- **Journal Config**: `config/journals.json` (may contain outdated info - verify against website)

---

**This file is the PRIMARY SOURCE OF TRUTH for journal information.**
